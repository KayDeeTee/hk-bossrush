    ...
    public class BossRushUpdate : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            if (BossRush.selectingStage)
            {
                ...
                BossRush.selectPos.anchoredPosition = new Vector2(((BossRush.selectX - 1) * 233), 18 - ((BossRush.selectY - 1) * 232));
            }
            ...
            if (BossRush.gm.gameState == GameState.PLAYING)
            {
                ...
                if (BossInfo.isBossDead() && BossRush.gm.sceneName != "Tutorial_01" && BossRush.gm.sceneName != "Dream_04_White_Defender" && BossRush.gm.sceneName != "Dream_Mighty_Zote" && BossRush.gm.sceneName != "Dream_01_False_Knight" && BossRush.gm.sceneName != "Dream_02_Mage_Lord" && BossRush.gm.sceneName != "Dream_03_Infected_Knight" && BossRush.gm.sceneName != "Grimm_Nightmare")
                {
                    if (!spawnedItems)
                    {
                        HeroController.instance.RelinquishControl();
                        HeroController.instance.StopAnimationControl();
                        HeroController.instance.cState.invulnerable = true;
                        ModInventory.PaneHandler.showMenu();
                        BossRush.pickups = 0;
                        spawnedItems = true;
                    }
                    else
                    {
                        ...
                        */
                        if (BossRush.pickups > (BossRush.flawless ? 1 : 0))
                        {
                            if (!BossRush.selectingStage)
                            {
                                ModInventory.PaneHandler.hideMenu();
